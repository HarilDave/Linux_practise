                         ...
                         size_t len, loff_t *off)
{
    ...
    dev->write_count++;       // NEW → increment counter

    printk(KERN_INFO "[mydev%d] wrote: %s (count=%d)\n",
                        ...
                        size_t len, loff_t *off)
{
    ...
    return simple_read_from_buffer(buf, len, off, temp, ret);
}
...
static int __init multi_dev_init(void)
{
    ...
    for (i = 0; i < NUM_DEVICES; i++) {
        ...
        my_devices[i].write_count = 0;   // NEW
        memset(my_devices[i].buffer, 0, BUF_SIZE);
        ...
        device_create(my_class, NULL, devno, NULL,
                      "%s%d", DEVICE_NAME, i);
