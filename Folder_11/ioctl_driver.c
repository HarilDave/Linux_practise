...
#define IOCTL_RESET_BUFFER _IO(0, 0)
#define IOCTL_GET_COUNT    _IOR(0, 1, int)
#define IOCTL_SET_DEBUG    _IOW(0, 2, int)

static char buffer[BUFFER_SIZE];
static int write_count = 0;
static int debug_mode = 0;    // NEW FLAG
                         ...
                         size_t len, loff_t *off)
{
    ...
    if (debug_mode == 1) {  // EXTRA DEBUG
        printk(KERN_INFO "[%s] DEBUG: write_count=%d, len=%zu\n",
               DEVICE_NAME, write_count, len);
    }
...
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
        ...
        case IOCTL_SET_DEBUG:   // NEW CASE
            if (copy_from_user(&debug_mode, (int *)arg, sizeof(debug_mode)))
                return -EFAULT;
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("You");
MODULE_DESCRIPTION("Char driver with ioctl (RESET, GET_COUNT, SET_DEBUG)");
